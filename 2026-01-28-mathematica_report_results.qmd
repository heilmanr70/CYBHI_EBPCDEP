---
title: "Mathematica Report Results-01-28-2026"
...
The following table shows high survey satisfaction by quarter.
...
noms_child_raw <- read.csv("/Users/cristin/CWS Dropbox/Cristin Young/CYBHI Project/Data/eInsight Exports/Individual Assessments/NOMS-child.csv", na = c("-", "", "NA", "N/A", "0"), header = FALSE) 
...
noms_self <- noms_self %>% 
  ...
  mutate(
    ...
    n_answered = sum(!is.na(c_across(matches("^(q1|x).*_score$")))),
    total_score = sum(c_across(matches("^(q1|x).*_score$")), na.rm = TRUE)) %>% 
  ungroup() %>% 
  mutate(
    date_completed = as.Date(date_completed),
...
paired_outcomes_self <- noms_self_long %>% 
  group_by(client_system_id, question) %>% 
  arrange(date_completed, row_id, .by_group = TRUE) %>% 
  mutate(
    days_since_initial = as.integer(date_completed - first(date_completed)),
    initial_date = first(date_completed),
    initial_score = first(score),
    initial_any_missing_q1x = first(any_missing_q1x)
  ) %>% 
  # flagging follow-ups
  mutate(
    is_within3 = days_since_initial <= 3 & days_since_initial >= 0,
...
# paired data: choose BEST follow-up (not just last)
pairs_primary_long_self <- paired_outcomes_self %>% 
  filter(is_after3 == TRUE) %>% #filtered for assessments >3 days from initial
  group_by(client_system_id, question) %>% 
  # rank follow-ups: prefer usable (non-NA score), prefer complete assessment, then latest date
  arrange(
    is.na(score),          # FALSE (has score) first
    desc(n_answered),       # highest n_answered first
    desc(date_completed),  # latest date next
    desc(row_id),          # tie-break within date
    .by_group = TRUE
  ) %>% 
  slice(1) %>% 
  ungroup() %>%
  transmute(
    ...
    follow_up_score = score,
    days_since_initial,
    follow_up_row_id = row_id,
    follow_up_any_missing_q1x = any_missing_q1x
  )
```
...
#| warning: false
#| message: false
raw_pss <- read_xlsx("/Users/cristin/CWS Dropbox/Cristin Young/CYBHI Project/Data/eInsight Exports/Individual Assessments/PSS.xlsx", na = c("-", "", "NA", "N/A"), col_names = FALSE) 
...
tab <- all_data %>% 
  #filter(!is.na(client_type)) %>% 
         filter(!is.na(gender_identity_num)) %>% 
  distinct(client_system_id, .keep_all = TRUE) %>% 
  tabyl(client_type, gender_identity_num) %>% 
  adorn_totals("row") %>% 
  adorn_percentages("row") %>%
  adorn_pct_formatting(digits = 0) %>%
  adorn_ns() 
