...
[x] Check variables listed to see if there are any obviously wrong ones / if they seem “correct”

[x] how many records have start and end data?
[x] send list to Natalie and Rikke of providers - with affected clients
[x] check before/after just on these clients
...
affected_clients_with_start_date <- affected_all_complete %>% 
  select(client_id, provider_id, intervention_start) %>% 
  distinct(client_id, provider_id)

write_csv(affected_clients_with_start_date, "/Users/cristin/Dropbox/Rebecca and Tristan and Cristin/EInsight Data/affected_clients_with_start_date.csv")
...
updated_all_data_complete <- updated_all_data %>% 
  filter(!is.na(intervention_start))

intervention_start_data <- discharge_data %>% 
  filter(!is.na(intervention_start))

all_clients <- intervention_start_data %>% distinct(client_id, provider_id)
...
write_csv(percent_by_grantee, "/Users/cristin/Dropbox/Rebecca and Tristan and Cristin/EInsight Data/percent_affected_grantees_with_start_date.csv")
```
