...
discharge_data <- read_xlsx("/Users/cristin/CWS Dropbox/Cristin Young/CYBHI Project/Data/eInsight Exports/Discharge Report Export/2026-02-10-discharge_report.xlsx", na = c("-", "", "NA", "N/A")) %>% 
  janitor::clean_names() 
...
demographic_data <- readxl::read_xlsx("/Users/cristin/CWS Dropbox/Cristin Young/CYBHI Project/Data/eInsight Exports/Admin Report Exports/2026-02-10-client_demographics.xlsx", na = c("-", "", "NA", "N/A")) %>% 
  janitor::clean_names()

demographic_data_new <- readxl::read_xlsx("/Users/cristin/CWS Dropbox/Cristin Young/CYBHI Project/Data/eInsight Exports/Admin Report Exports/2026-01-16-client_demographics.xlsx", na = c("-", "", "NA", "N/A")) %>%
janitor::clean_names()
...
length(unique(demographic_data$client_system_id))
```
...
## DEMOGRAPHIC DATA - Recoding ethnicity
1. Remove “don’t know”, “prefer not to answer”, and “not applicable” from the comma-separated responses.
2. Then recode the remaining responses:
- 0 if nothing is left after removal (or NA)
- 1 if exactly one valid response remains
- 2 if 2+ valid responses remain
3. do an analysis to look within ethnicity: so duplicated --> single ethnicity dummy variable
```{r}
# Define coding for single responses
...
demographic_data_filtered <- demographic_data_filtered %>% 
  ...
  mutate(
    recoded_ethn = case_when(
      is.na(cleaned_ethn) ~ NA,             # no valid response
      str_detect(cleaned_ethn, ",") ~ 2L,   # multiple responses
      TRUE ~ 1L                             # single response
    ),
    # assign numeric code to single values, 99 if 2+ values
    ethn_num = case_when(
      ...
      TRUE ~ unname(codebook_eth[cleaned_ethn])  # lookup from codebook
    )
  )
```
...
#cleaning funding rounds for specific providers
# monterey health dept = round 2
# children of the rainbow = round 2
# youth for change = round 1
# homeless children's network = round 2
# placer county office of education = round 1 
...
all_data <- left_join(discharge_data_filtered_clean, demographic_data_filtered, by = c("client_system_id", "provider_id", "provider_system_id"))
```
...
write_csv(discharge_data_filtered_clean, "/Users/cristin/CWS Dropbox/Cristin Young/CYBHI Project/Rebecca and Tristan and Cristin/EInsight Data/discharge_data_clean.csv")
```
