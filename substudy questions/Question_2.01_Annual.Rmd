...
discharge_data <- read_csv("/Users/cristin/CWS Dropbox/Cristin Young/CYBHI Project/Rebecca and Tristan and Cristin/EInsight Data/discharge_data_clean.csv", show_col_types = FALSE)
...
# load in spss data
spss <- haven::read_spss("/Users/cristin/CWS Dropbox/Cristin Young/CYBHI Project/Data/MASTER Grantee Files/Quant Team File/MasterGranteeSpreadsheet_forBHESQuantTeam_1-20-26_NEW!.sav") %>% 
  zap_missing() %>%
...
discharge_small <- discharge_data %>% 
...
demo_spss <- left_join(demo_discharge, spss, by = c("provider_id" = "2grantee_provider_id")) %>% 
  distinct(client_system_id, .keep_all = TRUE)
...
ggplot(ca_regions_demo_data) +
  ...
  labs(
    title = "Clients Served by Region",
    fill = "Clients"
  ) +
  theme_minimal() +
  theme(
