...
dot_and_arrow_plot_4_5 <- ggplot(df, aes(y = group)) + 
  ...
  #initial
  geom_point(aes(x = initial), size = 12, color = "#8A8C8E") + 
  geom_text(aes(x = initial, label = paste0(round(initial), "%")),
            size = 3.5, color = "grey20") +
  #followup
  geom_point(aes(x = followup), size = 12, color = "#BBDB49")+
...
#| warning: false
#| message: false
dot_and_arrow_plot_q2_child <- score_diff_child_q2 %>% 
  filter(!is.na(initial_total),
         !is.na(followup_total),
         !is.na(positive_outcome)) %>% 
  mutate(direction = case_when(
    positive_outcome == 1 ~ "Improved",
    positive_outcome == 0 ~ "Declined",
...
ggplot(aes(y = system_id)) +
  geom_segment(
    aes(
      x = initial_total, xend = followup_total,
      yend = system_id, color = direction
    ),
    arrow = arrow(length = unit(0.2, "cm"))
  ) +
  geom_point(aes(x = initial_total), size = 1, color = "grey30") +
  geom_point(aes(x = followup_total), size = 1, color = "grey30") +
  scale_color_manual(
    values = c("Improved" = "forestgreen",
               "Declined" = "firebrick",
               "No Change" = "gray50")
  ...
  labs(
    x = "Score",
    y = "Client",
    title = "Change in direction of Q2 score: Child"
  ) +
  theme_minimal()+
  theme(
     axis.title = element_text(size = 9),
...
#| warning: false
#| message: false
q2_self_score_long <- pairs_primary_long_q2_self %>%
  ungroup() %>% 
  select(client_system_id, initial_score, follow_up_score) %>% 
  pivot_longer(
    cols = c(initial_score, follow_up_score),
...
#| warning: false
#| message: false
dot_and_arrow_plot_q2_self <- score_diff_self_q2 %>% 
  filter(!is.na(initial_total),
         !is.na(followup_total),
         !is.na(positive_outcome)) %>% 
  mutate(direction = case_when(
...
ggplot(aes(y = client_system_id)) +
  ...
  labs(
    x = "Score",
    y = "Client",
    title = "Change in direction of Q2 score: Self"
  ) +
  theme_minimal()+
  theme(
     axis.title = element_text(size = 9),
...
#| message: false
# % of those who selected 4 or higher on question 1
tab5 <- pairs_primary_long_q2_self %>% 
  ungroup() %>% 
  summarise(
    initial_4_5 = round(100 * (mean(initial_score[!is.na(initial_score)]  %in% c(4,5), na.rm = TRUE)), 2),
    followup_4_5 = round(100 * (mean(follow_up_score[!is.na(follow_up_score)]  %in% c(4,5), na.rm = TRUE)), 2)
...
dot_and_arrow_plot_4_5_q2 <- ggplot(dfq2, aes(y = group)) + 
  ...
  labs(
    x = "Score",
    y = "",
    title = "Change in direction of scores for those who selected 4 or higher on NOMS Q2 by subgroup") + 
  theme_minimal()+
  theme(
     axis.title = element_text(size = 9),
     axis.text = element_text(size = 9),
...
#| warning: false
#| message: false
q3_child_score_long <- pairs_primary_long_q3 %>%
  ungroup() %>% 
  select(system_id, initial_score, follow_up_score) %>% 
  pivot_longer(
    cols = c(initial_score, follow_up_score),
...
#| warning: false
#| message: false
dot_and_arrow_plot_q3_child <- score_diff_child_q3 %>% 
  filter(!is.na(initial_total),
         !is.na(followup_total),
         !is.na(positive_outcome)) %>% 
  mutate(direction = case_when(
...
ggplot(aes(y = system_id)) +
  ...
  labs(
    x = "Score",
    y = "Client",
    title = "Change in direction of Q3 score: Child"
  ) +
  theme_minimal()+
  theme(
     axis.title = element_text(size = 9),
...
#| warning: false
#| message: false
q3_self_score_long <- pairs_primary_long_q3_self %>%
  ungroup() %>% 
  select(client_system_id, initial_score, follow_up_score) %>% 
  pivot_longer(
    cols = c(initial_score, follow_up_score),
...
#| warning: false
#| message: false
dot_and_arrow_plot_q3_self <- score_diff_self_q3 %>% 
  filter(!is.na(initial_total),
         !is.na(followup_total),
         !is.na(positive_outcome)) %>% 
  mutate(direction = case_when(
...
ggplot(aes(y = client_system_id)) +
  ...
  labs(
    x = "Score",
    y = "Client",
    title = "Change in direction of Q3 score: Self"
  ) +
  theme_minimal()+
  theme(
     axis.title = element_text(size = 9),
...
#| message: false
# % of those who selected 4 or higher on question 1
tab5 <- pairs_primary_long_q3_self %>% 
  ungroup() %>% 
  summarise(
    initial_4_5 = round(100 * (mean(initial_score[!is.na(initial_score)]  %in% c(4,5), na.rm = TRUE)), 2),
    followup_4_5 = round(100 * (mean(follow_up_score[!is.na(follow_up_score)]  %in% c(4,5), na.rm = TRUE)), 2)
...
dot_and_arrow_plot_4_5_q3 <- ggplot(dfq3, aes(y = group)) + 
  ...
  labs(
    x = "Score",
    y = "",
    title = "Change in direction of scores for those who selected 4 or higher on NOMS Q3 by subgroup") + 
  theme_minimal()+
  theme(
     axis.title = element_text(size = 9),
     axis.text = element_text(size = 9),
...
#| warning: false
#| message: false
q4_child_score_long <- pairs_primary_long_q4 %>%
  ungroup() %>% 
  select(system_id, initial_score, follow_up_score) %>% 
  pivot_longer(
    cols = c(initial_score, follow_up_score),
...
score_diff_child_q4 %>% 
  ungroup() %>% 
  summarise(
    mean_score = mean(initial_total, na.rm = TRUE),
    median_initial_score = median(initial_total, na.rm = TRUE),
    sd_score = sd(initial_total, na.rm = TRUE),
    n = sum(!is.na(initial_total))
  )
