...
## 3.03/3.04 Improved mental health (NOMS)
...
## 3.05/3.06 Improved daily life functioning (NOMS)
...
The percent of those who selected ratings of 4 or higher (excellent or very good) across Q2 at initial and discharge time points were:

```{r}
#| echo: false
#| warning: false
#| message: false
# % of those who selected 4 or higher on question 2
...
## 3.07/3.08Improved well-being (NOMS)
...
## 3.09/3.10 Improved social connectedness (NOMS)
...
#| message: false
# % of those who selected 4 or higher on question 4
tab5 <- pairs_primary_long_q4_self %>% 
  ungroup() %>% 
  summarise(
    initial_4_5 = round(100 * (mean(initial_score[!is.na(initial_score)]  %in% c(4,5), na.rm = TRUE)), 2),
    followup_4_5 = round(100 * (mean(follow_up_score[!is.na(follow_up_score)]  %in% c(4,5), na.rm = TRUE)), 2)
...
dot_and_arrow_plot_4_5_q4 <- ggplot(dfq4, aes(y = group)) + 
  ...
  labs(
    x = "Score",
    y = "",
    title = "Change in direction of scores for those who selected 4 or higher on NOMS Q4 by subgroup") + 
  theme_minimal()+
  theme(
    axis.title = element_text(size = 9),
    axis.text = element_text(size = 9),
